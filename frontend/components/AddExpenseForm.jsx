...
function AddExpenseForm({ onAdd }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 rounded-2xl shadow p-4 mb-4">
      <h3 className="text-lg font-semibold mb-2 text-gray-100">Add Expense</h3>

      {error && <p className="text-red-400 text-sm mb-2">{error}</p>}

      <div className="grid grid-cols-2 gap-2">
        <input type="date" name="date" value={form.date} onChange={handleChange}
          className="border border-gray-600 rounded p-2 w-full bg-gray-700 text-gray-100" required />
      ...
      <button type="submit"
        className="mt-3 bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded w-full">
        ➕ Add Expense
      </button>
    </form>
  );
